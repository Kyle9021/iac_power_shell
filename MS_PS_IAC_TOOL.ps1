...
While($pcee_report_selection -ne "Y" ){
   $pcee_report_selection = read-host "Want to save a detailed report? (Y/N)"
    Switch ($pcee_report_selection) 
        { 
            Y {Write-host "Creating Report in this Directory"
               Invoke-RestMethod `
                   -Method GET `
                   -Uri $($pcee_api_url + '/iac/v2/scans/' + $pcee_scan_id + '/export?filter=relative&timeUnit=hour&timeAmount=1&groupBy=assetName&fmt=csv') `
                   -Headers $pcee_scan_headers `
                   -Outfile ("./IaC_Scan_Report" + $pcee_timestamp + ".csv")
              } 
